...
void robmovil_ekf::LandmarkDetector::on_laser_scan(const sensor_msgs::msg::LaserScan::SharedPtr msg)
{
  ...
  for (int i = 0; i < msg->ranges.size(); i++)
  {
    ...
    tf2::Vector3 p(cos(angle)*range,sin(angle)*range,0);
  ...
  for (int i = 0; i < cartesian.size(); i++)
  {
    ...
    for (int j = i+1; j < cartesian.size(); j++)
    {
      tf2::Vector3 prev = landmark_points.back();
      double d = dist2(prev.getX(), prev.getY(), cartesian[j].getX(), cartesian[j].getY());

      if (d < LANDMARK_DIAMETER) {
    ...
    tf2::Vector3 lmin = landmark_points[0];
    double min_dist = sqrt(lmin.getX()*lmin.getX() + lmin.getY()*lmin.getY());
    for (const auto& q : landmark_points) {
      double d = sqrt(q.getX()*q.getX() + q.getY()*q.getY());
      if (d < min_dist) {
        min_dist = d;
        lmin = q;
      }
    ...
    centroid = lmin + lmin.normalized() * (LANDMARK_DIAMETER / 2.0);
