...
bool KinematicPositionController::control(const rclcpp::Time& t, double& v, double& w)
{
  ...
  double g_theta_i = - goal_a; 
  
  double dx = cos(goal_a)*dx_i + sin(goal_a)*dy_i; 
  double dy = -sin(goal_a)*dx_i + cos(goal_a)*dy_i; 
  double theta = angles::normalize_angle(current_a - goal_a);

  // Computar variables del sistema de control
  double rho = sqrt(dx*dx + dy*dy); 
  double alpha = angles::normalize_angle(std::atan2(dy, dx)- theta); // Normalizes the angle to be -M_PI circle to +M_PI circle It takes and returns radians. 
