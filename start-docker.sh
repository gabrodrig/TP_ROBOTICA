...
start(){
        xhost +local:docker
        chmod -R 755 ./volume
	run "docker run --name ros2_robotica --shm-size=256m -v ./volume:/root/ros2_ws/src/robotica --env DISPLAY=$DISPLAY --volume /tmp/.X11-unix:/tmp/.X11-unix:rw --env QT_X11_NO_MITSHM=1 --user $(id -u):$(id -g) -it -d ros2_robotica "
