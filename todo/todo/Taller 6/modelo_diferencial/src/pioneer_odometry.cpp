...
void PioneerOdometry::on_velocity_cmd(const geometry_msgs::msg::Twist::SharedPtr twist)
{
  ...
  const double linear_v = twist->linear.x;
  const double angular_v = twist->angular.z;

  double vLeft = (linear_v - 0.5 * WHEEL_BASELINE * angular_v)/WHEEL_RADIUS;
  double vRight = (linear_v + 0.5 * WHEEL_BASELINE * angular_v)/WHEEL_RADIUS;
...
void PioneerOdometry::on_encoder_ticks(const robmovil_msgs::msg::EncoderTicks::SharedPtr encoder)
{
  ...
  double preCalc = (M_PI*WHEEL_RADIUS*2)/ENCODER_TICKS;
  double dIzq = delta_ticks_left*preCalc;
  double dDer = delta_ticks_right*preCalc;

  double d = (dIzq + dDer)/2;
  double delta_theta = (dDer - dIzq)/(WHEEL_BASELINE);
